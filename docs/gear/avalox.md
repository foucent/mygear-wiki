---
source_url: https://mp.weixin.qq.com/s/ZCHySmfX1gA7-0fCEUMeew
source_title: "阿瓦拉 P500底板 横板ST柄"
imported: 2026-07-23
description: "Avalox table tennis blades — classic P500, YouTube review, and where to buy on Amazon and AliExpress."
---
# Avalox Table Tennis Blades

**Avalox** is a classic blade brand founded in **1987** by partners from Sweden, Japan, and Malaysia. Its **Swedish-made** blades — built with the traditional method where adhesive slowly seeps into the wood under controlled temperature and humidity — helped champions like **Kong Linghui**, **Wang Nan**, and **Wang Tao** win world titles in the 90s.

---

## P500 — The Legend

**Avalox P500** — a classic 5-ply construction (**Koto** top layer, **Tanne** middle, **Ayous** center, about **5.7 mm** thick). The brown handle and red "cat's eye" cosmetics were once near-standard in many professional environments.

<div class="mg-gallery mg-gallery--gear-album" markdown="0">

  <a href="/images/avalox-p500/01.jpg"><img src="/images/avalox-p500/01.jpg" alt="Avalox P500 table tennis blade photo 1" loading="lazy"></a>

  <a href="/images/avalox-p500/02.jpg"><img src="/images/avalox-p500/02.jpg" alt="Avalox P500 table tennis blade photo 2" loading="lazy"></a>

  <a href="/images/avalox-p500/03.jpg"><img src="/images/avalox-p500/03.jpg" alt="Avalox P500 table tennis blade photo 3" loading="lazy"></a>

  <a href="/images/avalox-p500/04.jpg"><img src="/images/avalox-p500/04.jpg" alt="Avalox P500 table tennis blade photo 4" loading="lazy"></a>

</div>

## Related videos on YouTube

<div style="position:relative;padding-top:56.25%"><iframe src="https://www.youtube.com/embed/YNaKFjwz70I" style="position:absolute;top:0;left:0;width:100%;height:100%" frameborder="0" allowfullscreen></iframe></div>

---

!!! tip "Related"

    Fiber placement: [Outer vs Inner Fiber](../guide/outer-vs-inner-fiber.md). Live USD references: [Blades](../blades.md). Photo gallery by brand: [Brand galleries](../index.md#brands).
